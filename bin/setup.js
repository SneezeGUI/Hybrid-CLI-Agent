#!/usr/bin/env node
/**
 * Interactive Setup Wizard for Gemini Worker MCP Server
 *
...
function banner() {
  console.log(`
${colors.cyan}╔══════════════════════════════════════════════════════════════╗
║       ${colors.bright}Gemini Worker MCP Server - Setup Wizard${colors.reset}${colors.cyan}             ║
╠══════════════════════════════════════════════════════════════╣
║  Configure API keys, authentication, and Claude integration  ║
╚══════════════════════════════════════════════════════════════╝${colors.reset}
`);
}
...
function saveEnvFile(env) {
  const lines = [
    '# Gemini Worker MCP Server Configuration',
  ...
  log(`\n✅ Configuration saved to ${ENV_FILE}`, colors.green);
}
...
async function runSetup() {
  ...
  if (geminiInstalled) {
    log('   ✅ Gemini CLI is installed', colors.green);
  } else {
    log('   ⚠️  Gemini CLI not found. Install from: https://github.com/google-gemini/gemini-cli', colors.yellow);
  ...
  if (geminiInstalled) {
    ...
    if (oauthAuthenticated) {
      log('   ✅ OAuth is authenticated (gemini auth login)', colors.green);
    } else {
      log('   ⚠️  OAuth not authenticated. Run: gemini auth login', colors.yellow);
    }
  }
      ...
      choices: [
        {
          name: oauthAuthenticated
            ? '🔐 OAuth (gemini auth login) - ✅ AUTHENTICATED - FREE, recommended'
            : '🔐 OAuth (gemini auth login) - FREE, recommended for Pro/Ultra subscribers',
          value: 'oauth'
        },
        { name: '🔑 Gemini API Key - Get from makersuite.google.com', value: 'api-key' },
        { name: '☁️  Vertex AI - For enterprise/higher limits', value: 'vertex' },
        { name: '⏭️  Skip - I\'ll configure manually later', value: 'skip' },
      ],
      default: defaultAuth,
    },
  ]);
  ...
  if (authMethod === 'oauth') {
    log('\n📝 OAuth selected. Make sure you\'ve run: gemini auth login', colors.cyan);
  ...
  if (authMethod !== 'skip') {
    ...
    if (testAuth) {
      log('\n🧪 Testing Gemini authentication...', colors.blue);
      const result = await testGeminiAuth();
      if (result.success) {
        log('   ✅ Authentication successful!', colors.green);
      } else {
        log('   ❌ Authentication failed. Check your credentials.', colors.red);
  ...
  if (registerClaude) {
    ...
    settings.mcpServers['gemini-worker'] = {
      command: 'node',
      args: [MCP_SERVER_SCRIPT],
      cwd: PROJECT_ROOT,
      env: {
        // Pass through relevant env vars
        ...(newEnv.OPENROUTER_API_KEY && { OPENROUTER_API_KEY: newEnv.OPENROUTER_API_KEY }),
      },
    };

    saveClaudeSettings(settings);
    log(`\n✅ Registered with Claude Code at ${CLAUDE_SETTINGS_FILE}`, colors.green);
  ...
  console.log(`
${colors.cyan}╔══════════════════════════════════════════════════════════════╗
║                    ${colors.bright}Setup Complete!${colors.reset}${colors.cyan}                            ║
╚══════════════════════════════════════════════════════════════╝${colors.reset}
